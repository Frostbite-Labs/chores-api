...
- `docs/openapi.yaml` / `docs/openapi.json` - generated, machine-readable contract. Regenerated by `npm run openapi` from `api/scripts/gen-openapi.ts`. **Every new or changed endpoint must be reflected in the generator and the YAML regenerated as part of the same change** — see "OpenAPI coverage" below.
...
Every route under `src/modules/*/*.routes.ts` has a corresponding entry in `api/scripts/gen-openapi.ts`. **When you add, remove, or change a route, update the generator in the same change and re-run `npm run openapi`** so `docs/openapi.yaml` and `docs/openapi.json` stay authoritative. The script:
